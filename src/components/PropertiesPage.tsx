...
export default function PropertiesPage({ properties }: { properties: SanityProperty[] }) {
  ...
  return (
    <>
      ...
      {/* ── PAGE HEADER ────────────────────────────────────── */}
      <section style={{ background: '#002b40', paddingTop: '9rem', paddingBottom: '5rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end',
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#e94e1b',
              ...
              <h1 style={{ fontSize: 'clamp(2.5rem,5vw,4rem)', fontWeight: 900,
                ...
                Aktuální<br/>
                <span style={{ color: '#e94e1b' }}>nemovitosti.</span>
              </h1>
            </div>
      ...
      <section style={{ background: '#f4f3f0', minHeight: '60vh', paddingBottom: '8rem' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 2rem' }}>
          ...
          <div style={{ position: 'sticky', top: 64, zIndex: 40, background: '#f4f3f0',
            ...
            borderBottom: '1px solid rgba(0,43,64,0.08)',
            marginBottom: '3rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'center',
              ...
              <div style={{ flex: '1 1 240px', position: 'relative' }}>
                ...
                <input type="text" placeholder="Hledat (název, lokalita, dispozice…)"
                  ...
                  style={{ width: '100%', paddingLeft: '2.25rem', paddingRight: '1rem',
                    ...
                    fontSize: '0.82rem', fontWeight: 500, color: '#002b40',
                    background: '#fff', border: '1px solid rgba(0,43,64,0.12)',
                    borderRadius: 999, outline: 'none',
                    boxSizing: 'border-box', fontFamily: 'inherit', transition: 'border-color 0.2s' }}
                  onFocus={e => (e.currentTarget.style.borderColor = '#e94e1b')}
                  onBlur={e => (e.currentTarget.style.borderColor = 'rgba(0,43,64,0.12)')}
                />
              </div>
              ...
              <button onClick={() => setFiltersOpen(o => !o)}
                style={{ display: 'flex', alignItems: 'center', gap: '0.5rem',
                  background: activeCount > 0 ? '#e94e1b' : '#fff',
                  color: activeCount > 0 ? '#fff' : '#002b40',
                  border: '1px solid rgba(0,43,64,0.12)',
              ...
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <select value={sort} onChange={e => setSort(e.target.value as SortKey)}
                  style={{ appearance: 'none', background: '#fff',
                    ...
                    fontSize: '0.78rem', fontWeight: 600, color: '#002b40',
                ...
                <svg style={{ position: 'absolute', right: '0.75rem', top: '50%',
                  ...
                  <path d="M2 3.5l3 3 3-3" stroke="#002b40" strokeWidth="1.4" strokeLinecap="round"/>
                </svg>
              </div>
              ...
              {activeCount > 0 && (
                <button onClick={resetFilters}
                  style={{ background: 'none', border: 'none', padding: 0,
                    fontSize: '0.72rem', fontWeight: 600, color: 'rgba(0,43,64,0.35)',
                    cursor: 'pointer', letterSpacing: '0.05em', textTransform: 'uppercase',
                    transition: 'color 0.2s', flexShrink: 0 }}
                  onMouseEnter={e => (e.currentTarget as HTMLElement).style.color = '#e94e1b'}
                  onMouseLeave={e => (e.currentTarget as HTMLElement).style.color = 'rgba(0,43,64,0.35)'}>
                  Zrušit filtry
                </button>
              )}
            </div>
          ...
          <div style={{ marginTop: '5rem', padding: '3rem 3.5rem',
            background: '#002b40', borderRadius: '1.5rem',
            ...
            <button onClick={openContactModal}
              style={{ background: '#e94e1b', color: '#fff', border: 'none',
                ...
                transition: 'all 0.25s', boxShadow: '0 0 30px rgba(233,78,27,0.3)', flexShrink: 0 }}
...
function PropertyRow({ p, index, total }: { p: SanityProperty; index: number; total: number }) {
  ...
  return (
    <div>
      {index > 0 && <div style={{ height: 1, background: 'rgba(0,43,64,0.07)' }} />}
      ...
      <Link href={`/nemovitosti/${p.slug.current}`}
        ...
        <div style={{ display: 'grid',
          ...
          <div style={{ width: 80, height: 60, borderRadius: '0.6rem', flexShrink: 0,
            background: 'rgba(0,43,64,0.06)', overflow: 'hidden',
            ...
            {thumbUrl ? (
              ...
              <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                <path d="M2 17l5-6 3.5 3.5L15 9l5 8H2z" stroke="rgba(0,43,64,0.2)"
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: '1rem', fontWeight: 800, color: '#002b40',
              ...
              ...(hovered ? { color: '#e94e1b' } : {}) }}>
              {p.title}
            </div>
            <div style={{ fontSize: '0.75rem', color: 'rgba(0,43,64,0.4)',
              fontWeight: 500, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span>{p.location}</span>
              {p.area && <><span style={{ color: 'rgba(0,43,64,0.2)' }}>·</span><span>{p.area} m²</span></>}
              {p.layout && <><span style={{ color: 'rgba(0,43,64,0.2)' }}>·</span><span>{p.layout}</span></>}
            </div>
          </div>
      ...
      {index === total - 1 && <div style={{ height: 1, background: 'rgba(0,43,64,0.07)' }} />}
    </div>
  )
}
...
function EmptyState({ type, onReset }: { type: 'no-data' | 'no-results'; onReset: () => void }) {
  return (
    <div style={{ textAlign: 'center', paddingTop: '6rem', paddingBottom: '6rem' }}>
      {type === 'no-data' ? (
        <>
          <p style={{ fontSize: '1.2rem', fontWeight: 700, color: '#002b40', marginBottom: '0.5rem' }}>
            Žádné nemovitosti
          </p>
          <p style={{ fontSize: '0.85rem', color: 'rgba(0,43,64,0.4)' }}>
            Sanity CMS ještě neobsahuje žádné záznamy nebo se nepodařilo načíst data.
          </p>
          <button onClick={openContactModal}
            style={{ marginTop: '1.5rem', background: '#e94e1b', color: '#fff',
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
      <span style={{ fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.12em',
        textTransform: 'uppercase', color: 'rgba(0,43,64,0.3)', marginRight: '0.25rem', flexShrink: 0 }}>
        {label}
      </span>
      {children}
    </div>
  ...
  return (
    <button onClick={onClick}
      style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.04em',
        ...
        border: active ? 'none' : '1px solid rgba(0,43,64,0.12)',
        background: active ? '#e94e1b' : '#fff',
        color: active ? '#fff' : 'rgba(0,43,64,0.55)',
